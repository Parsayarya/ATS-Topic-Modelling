...
SW_list = ['al', 'et', 'mt', 'ip', 'ofthe', 'ix', 'xxiii', 'use','used','february', 'january', 'december',
           ...
           "xiv", "xxi", "xix", "xxii", "xvii", "xxv", "xxvi", "xii", "xxiv", 'xxx', 'springer', 'press', 'wiley', 'medium', 'john', 'cambridge', 'journal', 'oxford', 'blackwell',
           'xvii', 'april', 'ukraine', 'chinese', 'russia', 'canadian', 'iceland',
...
def LDAV1(df, text_column, n_topics=30, n_top_words=25):
    ...
    lda = LatentDirichletAllocation(n_components=n_topics, learning_method='online', learning_offset=50., random_state=42,
                                    verbose=1, n_jobs=-1, max_iter=10, doc_topic_prior=0.1, topic_word_prior=0.005).fit(dtm) #TODO: Change Max_iter to be lower thaan 10 this time
...
df2, topics_df = LDAV1(df, 'text', n_topics=90, n_top_words=25)
df2.to_csv('Data/Output3-NewDB/90_LDA_Topic_Loading_ScoreNewTill23.csv',index=False)
topics_df.to_csv('Data/Output3-NewDB/90_LDA_Topic_WordsNewTill23.csv', index=False)
